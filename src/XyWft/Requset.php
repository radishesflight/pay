...
class Requset
{
    ...
    public function Request($config)
    {
        ...
        $this->reqHandler->setGateUrl($this->config->getConfig('url'));

        $sign_type = $this->config->getConfig('sign_type');

        if ($sign_type == 'MD5') {
            $this->reqHandler->setKey($this->config->getConfig('key'));
            ...
            $private_key = $this->config->getConfig('private_rsa_key');
            $public_key = $this->config->getConfig('public_rsa_key');
            $private_key = chunk_split($private_key, 64);
            $public_key = chunk_split($public_key, 64);
            $private_key = "-----BEGIN RSA PRIVATE KEY-----\n" . $private_key . "-----END RSA PRIVATE KEY-----";
            $public_key = "-----BEGIN PUBLIC KEY-----\n" . $public_key . "-----END PUBLIC KEY-----";
            $this->reqHandler->setRSAKey($private_key);
            $this->resHandler->setRSAKey($public_key);
            $this->reqHandler->setSignType($sign_type);
        }
    }
...
//        $this->reqHandler->setParameter('service','unified.trade.native');//接口类型：unified.trade.native   表示统一扫码
        ...
        $this->reqHandler->setParameter('service', $params['service']);
        if (isset($params['appid'])) {
            $this->reqHandler->setParameter('appid', $params['appid']);
        }
        $this->reqHandler->setParameter('mch_id', $this->config->getConfig('mchId'));//必填项，商户号，由平台分配
        $this->reqHandler->setParameter('version', $this->config->getConfig('version'));
        $this->reqHandler->setParameter('sign_type', $this->config->getConfig('sign_type'));
        $this->reqHandler->setParameter('out_trade_no', $params['out_trade_no']);
        $this->reqHandler->setParameter('body', $params['body']);
        $this->reqHandler->setParameter('attach', $params['attach']);
        $this->reqHandler->setParameter('total_fee', (string)$params['total_fee']);
        $this->reqHandler->setParameter('mch_create_ip', $params['mch_create_ip']);


        //通知地址，必填项，接收平台通知的URL，需给绝对路径，255字符内格式如:http://wap.tenpay.com/tenpay.asp

        $this->reqHandler->setParameter('notify_url', $this->config->getConfig('notify_url'));// 支付成功异步回调通知地址，目前默认是空格，商户在测试支付和上线时必须改为自己的，且保证外网能访问到
     ...
     * 提交退款
     */
    public function submitRefund($reqParam)
    {
        //$this->reqHandler->setReqParams($_POST,array('method'));
        // $reqParam = $this->reqHandler->getAllParameters();
        if (empty($reqParam['transaction_id']) && empty($reqParam['out_trade_no'])) {
        ...
        $this->reqHandler->setParameter('out_trade_no', $reqParam['out_trade_no']);
        $this->reqHandler->setParameter('out_refund_no', $reqParam['out_refund_no']);
        $this->reqHandler->setParameter('total_fee', (string)$reqParam['total_fee']);
        $this->reqHandler->setParameter('refund_fee', (string)$reqParam['refund_fee']);
        ...
        $this->reqHandler->setParameter('mch_id', $this->config->getConfig('mchId'));//必填项，商户号，由平台分配
        $this->reqHandler->setParameter('nonce_str', mt_rand());//随机字符串，必填项，不长于 32 位
        $this->reqHandler->setParameter('op_user_id', $this->config->getConfig('mchId'));//必填项，操作员帐号,默认为商户号
        $this->reqHandler->setParameter('sign_type', $this->config->getConfig('sign_type'));
        ...
        if ($this->pay->call()) {
            ...
            if ($this->resHandler->isTenpaySign()) {
                ...
                if ($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0) {
                    ...
                    if ($result_code == 0) {
                        return array('status' => 200, 'msg' => '退款成功,请查看', 'data' => $this->resHandler->getAllParameters());
                    } else {
                        return array('status' => 500, 'msg' => $this->resHandler->getAllParameters('err_msg'), 'data' => $result_code);
                    }
//                    Utils::dataRecodes('提交退款',$res);
     ...
     * 查询退款
     */
    public function queryRefund($reqParam)
    {
        //$this->reqHandler->setReqParams($_POST,array('method'));
...
//        }
        ...
        $this->reqHandler->setParameter('out_trade_no', $reqParam['out_trade_no']);
        $this->reqHandler->setParameter('out_refund_no', $reqParam['out_refund_no']);
        //$this->reqHandler->setParameter('refund_id',$reqParam['refund_id']);
...
//                    Utils::dataRecodes('查询退款',$res);
                    $refund_status_0 = $this->resHandler->getParameter('refund_status_0');
                    if ($refund_status_0 == 'SUCCESS') {
                        return array('status' => 200, 'msg' => '查询成功,已退款', 'data' => $this->resHandler->getAllParameters());
                    } else {
                        return array('status' => 500, 'msg' => $this->resHandler->getAllParameters('	err_msg'), 'data' => $refund_status_0);
                    }
