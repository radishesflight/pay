...
class ClientResponseHandler
{
    ...
    public function verifyRSASign()
    {
        ...
        // ✅ 3. 验签
        $sign = base64_decode($this->getParameter("sign"));

        if ($this->getParameter('sign_type') == 'RSA_1_1') {
            $result = openssl_verify($signPars, $sign, $res);
        } else if ($this->getParameter('sign_type') == 'RSA_1_256') {
            $result = openssl_verify($signPars, $sign, $res, OPENSSL_ALGO_SHA256);
        } else {
            throw new Exception("未知签名类型");
        }

        // ❌ PHP8 不要再用 openssl_free_key
